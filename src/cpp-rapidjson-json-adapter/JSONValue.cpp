...
JSONValue::JSONValue(JSONDocument& document, rapidjson::Value& value,
...
bool JSONValue::getBoolean() const { return m_value.GetBool(); }

void JSONValue::setBoolean(bool value) { m_value.SetBool(value); }

std::string JSONValue::getString() const { return m_value.GetString(); }

void JSONValue::setString(const std::string& value) {
  m_value.SetString(value.c_str(), m_allocator);
}

bool JSONValue::isInteger() const { return m_value.IsInt(); }

int JSONValue::getInteger() const { return m_value.GetInt(); }

double JSONValue::getDouble(double) const { return m_value.GetDouble(); }

void JSONValue::setInteger(int value) { m_value.SetInt(value); }

void JSONValue::setLong(long value) { m_value.SetInt64(value); }

void JSONValue::setLongLong(long long value) { m_value.SetInt64(value); }

void JSONValue::setDouble(double value) { m_value.SetDouble(value); }
...
void JSONValue::addMember(const std::string& name,
                          std::unique_ptr<IJSONValue> valueToAdd) {
  loadObjectMembers();
  ...
  std::unique_ptr<rapidjson::Value> freeValue =
      m_document.removeFreeValue(adapterValueToAdd->m_value);

  rapidjson::Value memberName(name.c_str(), m_allocator);
  m_value.AddMember(memberName, *freeValue, m_allocator);

  std::unique_ptr<IJSONMember> member = std::make_unique<JSONMember>(
      m_document, name.c_str(), m_value[name.c_str()], m_allocator);
...
void JSONValue::addArrayValue(std::unique_ptr<IJSONValue> valueToAdd) {
  ...
  auto adapterValueAdded =
      std::make_unique<JSONValue>(m_document, addedValue, m_allocator);
...
std::unique_ptr<IJSONValue> JSONValue::buildValue(Type type) const {
  ...
  auto newValue =
      std::make_unique<JSONValue>(m_document, *freeValue, m_allocator);
...
void JSONValue::loadObjectMembers() const {
  ...
  for (auto itr = m_value.MemberBegin(); itr != m_value.MemberEnd(); ++itr) {
    ...
    std::unique_ptr<IJSONMember> member = std::make_unique<JSONMember>(
        m_document, memberName, itr->value, m_allocator);
...
void JSONValue::loadArrayValues() const {
  ...
  for (auto itr = m_value.Begin(); itr != m_value.End(); ++itr) {
    std::unique_ptr<IJSONValue> value =
        std::make_unique<JSONValue>(m_document, *itr, m_allocator);
